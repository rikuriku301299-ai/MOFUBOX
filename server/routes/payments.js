...
// Commission rate by subscription plan — the discount is the main upgrade
// incentive for the paid plans (see routes/plans.js for the catalog).
const RATE_BY_PLAN = { free: 0.07, standard: 0.05, pro: 0.035 };
...
function verifyStripeSignature(payload, header, secret) {
  ...
  if (!parts.v1 || parts.v1.length !== expected.length) return false; // timingSafeEqual throws on length mismatch
...
function webhook(req, res, rawBody) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // Monetisation fulfilment (see routes/plans.js): activate the plan or
    // boost that this checkout paid for. Required lazily to avoid the
    // plans.js <-> payments.js circular require at module load.
    const meta = session.metadata || {};
    if (meta.kind === 'subscription' && meta.user_id) {
      const { PLANS } = require('./plans');
      if (PLANS[meta.plan]) {
        db.prepare('UPDATE users SET plan = ?, stripe_subscription_id = ? WHERE id = ?')
          .run(meta.plan, session.subscription || null, Number(meta.user_id));
      }
    } else if (meta.kind === 'boost' && meta.reel_id) {
      const { boostUntilFromNow } = require('./plans');
      db.prepare('UPDATE reels SET boost_until = ? WHERE id = ?').run(boostUntilFromNow(), Number(meta.reel_id));
    }
  }

  // Subscription lapsed or cancelled at Stripe — drop the breeder back to free.
  if (event.type === 'customer.subscription.deleted') {
    const subId = event.data.object.id;
    db.prepare("UPDATE users SET plan = 'free', stripe_subscription_id = NULL WHERE stripe_subscription_id = ?").run(subId);
  }
...
async function recordDeal(req, res, body) {
  ...
  const rate = RATE_BY_PLAN[user.plan] || RATE_BY_PLAN.free;
  const commission = Math.round(price * rate);
  const catName = String((body && body.catName) || '').trim();
  const label = catName ? `${catName}・` : '';
  const ratePct = +(rate * 100).toFixed(1);
  const description = `成約手数料（${label}成約額 ¥${price.toLocaleString()} の${ratePct}%）`;
  ...
  if (stripeConfigured()) {
    ...
    try {
      ...
      db.prepare(`INSERT INTO orders (user_id, stripe_session_id, description, amount, currency, status, kind)
        VALUES (?, ?, ?, ?, 'jpy', 'pending', 'commission')`).run(user.id, session.id, description, commission);
      ...
      return res.json(200, { price, commission, ratePct, live: true, split: true, url: session.url });
    } catch (e) {
  ...
  db.prepare(`INSERT INTO orders (user_id, description, amount, currency, status, kind)
    VALUES (?, ?, ?, 'jpy', 'recorded', 'commission')`).run(user.id, description, commission);
  ...
  res.json(200, { price, commission, ratePct, live: false, message: 'Stripe接続後は、お客様のお支払いから手数料が自動で天引きされます。' });
}

// GET /api/revenue — admin summary: totals per revenue stream + MRR.
function revenueSummary(req, res) {
  ...
  for (const row of db.prepare(`
    SELECT COALESCE(kind, 'commission') AS kind, COALESCE(SUM(amount),0) AS s, COUNT(*) AS c
    FROM orders WHERE status IN ('paid','recorded','pending') GROUP BY 1`).all()) {
    byKind[row.kind] = { total: row.s, count: row.c };
  }

  // MRR: sum of plan fees across breeders currently on a paid plan.
  const { PLANS } = require('./plans');
  let mrr = 0;
  let subscribers = 0;
  for (const row of db.prepare(`
    SELECT plan, COUNT(*) AS c FROM users
    WHERE role = 'breeder' AND plan IN ('standard','pro') GROUP BY plan`).all()) {
    mrr += (PLANS[row.plan] ? PLANS[row.plan].priceYen : 0) * row.c;
    subscribers += row.c;
  }
  const activeBoosts = db.prepare("SELECT COUNT(*) AS c FROM reels WHERE boost_until > datetime('now')").get().c;

  const recent = db.prepare(`
    SELECT o.description, o.amount, o.status, o.kind, o.created_at, u.kennel, u.name
  ...
  res.json(200, {
    ...
    byKind, mrr, subscribers, activeBoosts,
    commissionRates: RATE_BY_PLAN,
    stripeLive: stripeConfigured(),
    recent: recent.map(r => ({
      breeder: r.kennel || r.name || '—',
      description: r.description, amount: r.amount, status: r.status, kind: r.kind, at: r.created_at,
    })),
  });
}

module.exports = {
  createCheckoutSession, webhook, stripeConfigured, stripeRequest, stripeGet,
  recordDeal, revenueSummary, connectStart, connectStatus, RATE_BY_PLAN,
};
